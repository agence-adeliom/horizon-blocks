...
{
    private const string DEFAULT_ORDER = 'date.DESC';
    ...
    private const string QUICK_ACCESS_TAXONOMY_PREFIX = 'qat-';

    public const bool DISPLAY_VALUES_IN_MULTISELECT_LABEL = false;
    public const int MULTISELECT_LABEL_MAXLENGTH = 30;
    ...
    private function initSearchFilter(string $searchName, ?string $label = null, ?string $placeholder = null, int $level = 1): void
    {
    ...
    private function handleChoiceAll(?string $choiceAll = null, array &$workingFilters = [], ?string $filterName = null): void
    {
        if (!empty($choiceAll) && !empty($filterName)) {
        ...
        if (null !== $filtersData) {
            foreach ($filtersData as $filter) {
                ...
                $choiceAll = match ($type) {
                    FilterTypesEnum::META->value => !empty($filter[ListingBlock::FIELD_FILTERS_META_CHOICE_ALL])
                        ? $filter[ListingBlock::FIELD_FILTERS_META_CHOICE_ALL]
                        : null,
                    FilterTypesEnum::TAXONOMY->value => !empty($filter[ListingBlock::FIELD_FILTERS_TAX_CHOICE_ALL])
                        ? $filter[ListingBlock::FIELD_FILTERS_TAX_CHOICE_ALL]
                        : null,
                    default => null,
                };
    ...
    private function applyFilters(array $filtersToApply, QueryBuilder $qb, int $level = 1): void
    {
        $workingFilters = match ($level) {
            1 => $this->filters,
            2 => $this->secondaryFilters,
    ...
    private function applyForcedFilters(QueryBuilder $qb): void
    {
        ...
        if (is_array($this->forcedFilters)) {
            foreach ($this->forcedFilters as $forcedFilter) {
                if (
                    is_array($forcedFilter) &&
                    !empty($forcedFilter[ListingBlock::FIELD_FILTERS_TYPE]) &&
                    !empty($forcedFilter[ListingBlock::FIELD_FILTERS_TAXONOMY]) &&
                    !empty($forcedFilter[ListingBlock::FIELD_FILTERS_TAXONOMY_VALUE])
                ) {
    ...
    private function adaptQueryBuilderToListingCards(QueryBuilder $qb): void
    {
                ...
                if (!empty($innerCard[ListingBlock::FIELD_INNER_CARD_PAGES])) {
                    switch ($innerCard[ListingBlock::FIELD_INNER_CARD_PAGES]) {
                        ...
                        case ListingBlock::VALUE_INNER_CARD_PAGES_CUSTOM:
                            $card->setPages(
                                array_filter(array_map('intval', explode(',', $innerCard[ListingBlock::FIELD_INNER_CARD_CUSTOM_PAGES]))),
                            );
                            break;
        ...
        $oldPage = $qb->getPage();
        $qb->page(1);
        $totalCardsDisplayedPerPage = $this->getTotalCardsDisplayedPerPage(
            numberOfItems: $qb->getCount(),
            perPage: $qb->getPerPage(),
            cards: $cards,
        );
        $qb->page($oldPage);
        ...
        if (null !== $this->postType) {
            ...
            if (!empty($this->displayedInnerCards)) {
                ...
                foreach ($this->displayedInnerCards as $displayedInnerCard) {
                    ...
                    ArrayService::insertAtIndex(
                        $items,
                        $indexToHave,
                        $displayedInnerCard->toStdClass(currentPage: $this->page, perPage: $this->perPage),
                    );
                }
